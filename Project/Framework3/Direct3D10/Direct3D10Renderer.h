...
//	bool resetDevice();
	...
	ShaderID addShader(const char *vsText, const char *gsText, const char *fsText, const int vsLine, const int gsLine, const int fsLine,
		const char *header = NULL, const char *extra = NULL, const StreamDesc *streamOut = NULL, const uint nElements = 0, const char *fileName = NULL, const char **attributeNames = NULL, const int nAttributes = 0, const uint flags = 0);
	VertexFormatID addVertexFormat(const FormatDesc *formatDesc, const uint nAttribs, const ShaderID shader = SHADER_NONE);
	VertexBufferID addVertexBuffer(const long size, const BufferAccess bufferAccess, const void *data = NULL, const uint flags = 0);
	...
	int getVertexFormatSize(const VertexFormatID vertexFormat);
...
//	void changeTexture(const uint imageUnit, const TextureID textureID);
	...
	void changeVertexBuffers(const int streams, const int numStreams, const VertexBufferID *vertexBufferIDs, const intptr *offsets);
	void changeStreamTargets(const int numStreams, const VertexBufferID *vertexBufferIDs, const intptr *offset);
