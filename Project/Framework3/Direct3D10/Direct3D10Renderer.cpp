...
ShaderID Direct3D10Renderer::addShader(const char *vsText, const char *gsText, const char *fsText, const int vsLine, const int gsLine, const int fsLine,
									   const char *header, const char *extra, const StreamDesc *streamOut, const uint nElements, const char *fileName, const char **attributeNames, const int nAttributes, const uint flags){
	...
	if (gsText != NULL){
		...
		uint SOstride = 0;
		D3D10_SO_DECLARATION_ENTRY *SODec = new D3D10_SO_DECLARATION_ENTRY[nElements];
		...
		if (streamOut != NULL) {
			...
			for (uint i = 0; i < nElements; i++){
				int stream = streamOut[i].stream;
				uint startComponent = streamOut[i].startComponent;
				uint componentCount = streamOut[i].componentCount;
		
				SODec[i].SemanticName = semantics[streamOut[i].type];
				SODec[i].SemanticIndex = index[streamOut[i].type]++;
				SODec[i].StartComponent = startComponent;
				SODec[i].ComponentCount = componentCount;
				SODec[i].OutputSlot = stream;

				SOstride += componentCount*4;
			}
		}
		...
		if (SUCCEEDED(D3D10CompileShader(shaderString, shaderString.getLength(), fileName, NULL, NULL, "main", "gs_4_0", compileFlags, &shaderBuf, &errorsBuf))){
			...
			HRESULT result;
			void *pShader = shaderBuf->GetBufferPointer();
			uint size = shaderBuf->GetBufferSize();
			if (streamOut != NULL)
				result = device->CreateGeometryShaderWithStreamOutput(shaderBuf->GetBufferPointer(), shaderBuf->GetBufferSize(), SODec, nElements, SOstride, &shader.geometryShader);
			else
				result = device->CreateGeometryShader(shaderBuf->GetBufferPointer(), shaderBuf->GetBufferSize(), &shader.geometryShader);
...
VertexFormatID Direct3D10Renderer::addVertexFormat(const FormatDesc *formatDesc, const uint nAttribs, const ShaderID shader){
	...
	static const D3D10_INPUT_CLASSIFICATION classes[2] = {
	...
	for (uint i = 0; i < nAttribs; i++){
		...
		int streamClass = formatDesc[i].streamClass;
		...
		desc[i].InputSlotClass = classes[streamClass];
		desc[i].InstanceDataStepRate = (streamClass == CLASS_PER_INSTANCE) ? 1 : 0;
...
int Direct3D10Renderer::getVertexFormatSize(const VertexFormatID vertexFormat)
{
	int totalSize = 0;
	VertexFormat vf = vertexFormats[vertexFormat];

	//VF stores vertex sizes for each stream, simply combine them here
	for (int i = 0; i < 8; i++) {
		totalSize += vf.vertexSize[i];
	}
...
VertexBufferID Direct3D10Renderer::addVertexBuffer(const long size, const BufferAccess bufferAccess, const void *data, const uint flags){
	VertexBuffer vb;
	vb.size = size;
	...
	desc.ByteWidth = size;
	desc.BindFlags = D3D10_BIND_VERTEX_BUFFER | flags;
...
void Direct3D10Renderer::changeVertexBuffers(const int stream, const int numStreams, const VertexBufferID *vertexBufferIDs, const intptr *offsets){
	//if (vertexBufferID != currentVertexBuffers[stream] || offset != currentOffsets[stream]){
	UINT strides[MAX_VERTEXSTREAM];
	UINT vOffsets[MAX_VERTEXSTREAM];
	ID3D10Buffer* vBufs[MAX_VERTEXSTREAM];
		
	if (offsets == NULL) {
		intptr noOffset[MAX_VERTEXSTREAM];
		memset(noOffset, 0, sizeof(intptr)*MAX_VERTEXSTREAM);
		offsets = noOffset;
	}

	for(int i = 0; i < numStreams; i++) {
		strides[i] = vertexFormats[currentVertexFormat].vertexSize[stream+i];
		vOffsets[i] = (UINT) offsets[i];
		vBufs[i] = vertexBuffers[vertexBufferIDs[i]].vertexBuffer;
	}

	device->IASetVertexBuffers(stream, numStreams, vBufs, strides, vOffsets);
	
	for(int i = 0; i < numStreams; i++) {
		currentVertexBuffers[stream+i] = vertexBufferIDs[i];
		currentOffsets[stream+i] = offsets[i];
	}
	//}
}

void Direct3D10Renderer::changeStreamTargets(const int numStreams, const VertexBufferID *vertexBufferIDs, const intptr *offsets = NULL){
	ID3D10Buffer *SOBufs[8];
	intptr		 offset[8];
	
	if (vertexBufferIDs == NULL){
		device->SOSetTargets( 0, NULL, 0 );
		return;
	}

	if (offsets == NULL) {
		memset(offset, 0, sizeof(uint)*numStreams);
		offsets = offset;
	}

	for (int i = 0; i < numStreams; i++) {
		SOBufs[i] = vertexBuffers[vertexBufferIDs[i]].vertexBuffer;
	}

	device->SOSetTargets(numStreams, SOBufs, (UINT*)offsets);
}
...
void Direct3D10Renderer::drawPlain(const Primitives primitives, vec2 *vertices, const uint nVertices, const BlendStateID blendState, const DepthStateID depthState, const vec4 *color){
	...
	if (plainShader == SHADER_NONE){
		...
		FormatDesc format[] = { 0, TYPE_VERTEX, FORMAT_FLOAT, CLASS_PER_VERTEX, 2 };
...
void Direct3D10Renderer::drawTextured(const Primitives primitives, TexVertex *vertices, const uint nVertices, const TextureID texture, const SamplerStateID samplerState, const BlendStateID blendState, const DepthStateID depthState, const vec4 *color){
	...
	if (texShader == SHADER_NONE){
		...
		FormatDesc format[] = {
			0, TYPE_VERTEX,   FORMAT_FLOAT, CLASS_PER_VERTEX, 2,
			0, TYPE_TEXCOORD, FORMAT_FLOAT, CLASS_PER_VERTEX, 2, 
		};
